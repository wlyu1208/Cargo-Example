...
import com.example.cargoexample.util.getIntOrNull
...
@Repository
class CargoRepositoryImpl (private val jdbcTemplate: NamedParameterJdbcTemplate, private val carBrandRepository: CarBrandRepository) : CargoRepository {
    override fun getAll(pageIndex: Int): List<Cargo> = jdbcTemplate.query(
        """
           with c as ( select * from cargo order by id limit :limit offset :offset )
           select c.*, cb.title as brand from c
           join car_brand cb on c.branch_id = cb.id
           order by id
           """, mapOf(
            "limit" to PAGE_SIZE,
    ...
    override fun findById(id: Int): Cargo? = jdbcTemplate.query(
        """
            with c as (select * from cargo where id = :id)
            select c.*, cb.title as brand from c
